...
func _init() -> void:
	...
	if initialize_response['status'] > 0:
		W.print("Failed to init Steam! Shutting down. %s" % initialize_response)
		set_process(false)
	...
	W.print("Steam id: ", steam_id)
	W.print("Steam username: ", steam_username)

	if Steam.isSubscribed() == false:
		W.print("User does not own game!")
...
func join_lobby(lobby_id: Variant) -> Error:
	W.print("Joining lobby ", lobby_id)
...
func _on_lobby_created(status: int, lobby_id: int) -> void:
	W.print("On lobby created")
	if status == 1:
		W.print("Created lobby ", lobby_id)
...
func _on_lobby_joined(lobby: int, _permissions: int, _locked: bool, response: int) -> void:
	W.print("On lobby joined: ", response)
	if response == 1:
		var lobby_owner_id: int = Steam.getLobbyOwner(lobby)
		if lobby_owner_id != Steam.getSteamID():
			W.print("Connecting client to socket...")
			var error: Error = peer.create_client(lobby_owner_id, 0)
			if error == OK:
				W.print("Connecting peer to host...")
				multiplayer.multiplayer_peer = peer
			else:
				push_error("Error creating client: ", error)
	else:
		match response:
			2:  W.print("This lobby no longer exists.")
			3:  W.print("You don't have permission to join this lobby.")
			4:  W.print("The lobby is now full.")
			5:  W.print("Uh... something unexpected happened!")
			6:  W.print("You are banned from this lobby.")
			7:  W.print("You cannot join due to having a limited account.")
			8:  W.print("This lobby is locked or disabled.")
			9:  W.print("This lobby is community locked.")
			10: W.print("A user in the lobby has blocked you from joining.")
			11: W.print("A user you have blocked is in the lobby.")
